...
import yuluyao.frog.click.RetryListener;
import yuluyao.frog.diff.DiffCallback;
import yuluyao.frog.load.BaseLoadDecor;
import yuluyao.frog.load.DefaultLoadDecor;
import yuluyao.frog.multi.MultiEntity;
 ...
 * 时 间：2017/8/9 14:50
 */
public abstract class FrogAdapter<T, VH extends FrogHolder> extends RecyclerView.Adapter<VH> {
  ...
  @SuppressWarnings("unchecked") protected VH buildStaticHolder(View view) {
    ...
    // 泛型擦除会导致z为null
    if (z == null) {
      VH = (VH) new FrogHolder(view);
    } else {
      VH = createGenericKInstance(z, view);
    }
    return VH != null ? VH : (VH) new FrogHolder(view);
  }
    ...
    if (type instanceof ParameterizedType) {
      ...
      for (Type temp : types) {
        if (temp instanceof Class) {
          ...
          if (FrogHolder.class.isAssignableFrom(tempClass)) {
            return tempClass;
          }
        }
      }
